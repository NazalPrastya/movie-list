...
import { getAllTv, searchTv } from '../../../services/series.service';
          ...
          {movies.length > 0 &&
            movies.map((movie) => (
              <CardMovie key={movie.id} id={movie.id}>
                <CardMovie.Header image={movie.poster_path} title={movie.name} rating={movie.vote_average} />
                <CardMovie.Body title={movie.name} rating={movie.vote_average} date={movie.first_air_date} />
              </CardMovie>
            ))}
        </div>
      </div>
